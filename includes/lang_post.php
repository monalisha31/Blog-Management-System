...
if(isset($_GET['page'])){
  ...
  $number = 9;
  $query1 = "SELECT * FROM post WHERE P_lang_id=$lang_id AND P_status='published'";
$result1 = mysqli_query($connection, $query1);
$all_post = mysqli_num_rows($result1);
$total = ceil($all_post / $number);
$start  = ($page_id - 1)* $number;
$query = "SELECT * FROM post WHERE P_lang_id=$lang_id AND P_status='published' ORDER BY P_id DESC LIMIT $start , $number";
