<?php
  ...
  if(isset($_POST['contact_submit'])) {
    ...
    if(empty($name)) {
      ...
      header("Location: ../../page-contact.php?error=Name_is_required");
    }elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
      array_push($error, "<p class='alert alert-danger'>Email is invalid</p>");
      header("Location: ../../page-contact.php?error=Email_is_invalid");
    }elseif (empty($email)) {
      array_push($error, "<p class='alert alert-danger'>Email is required</p>");
      header("Location: ../../page-contact.php?error=Email_is_required");
    }
    else{
      if(empty($error)) {
